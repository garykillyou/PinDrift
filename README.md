...
> **更新版本時請整包換掉**：只覆蓋 `PinDrift.exe` 不夠，新版的修正常常在 `_internal` 裡
> （例如連線用的 OpenSSL DLL）。刪掉舊資料夾、換上新的，再把舊的兩個 `.json` 複製回來即可保留設定與最愛。
> 用壓縮檔交付時，請先完整解壓縮再執行，不要直接在壓縮檔裡雙擊 exe。
...
- **規劃路徑（沿實際道路）**：路線模式下按「規劃路徑」，然後在地圖上依序點選要經過的點
  ...
  **取代目前整條路線**。游標會變成十字準心，已選的點會依序標上「起」「2」「3」…，按鈕上也會
  ...
  - **簡化**下拉選單決定寫進座標表格的節點密度。路徑服務回傳的轉彎點動輒上百個
    ...
    想完全貼合真實道路就選「關閉」。
  - **簡化目前路線**按鈕用同一個簡化強度，把表格裡**現有**的路線再抽稀一次——自己在地圖上點的、
    從最愛載入的、KML 匯入的路線都能用。有備註的點（起點、終點、地名）一律保留。簡化後已走的
    進度會歸零（跟載入最愛一樣）；點數沒有減少就不會動到路線。簡化選「關閉」時按鈕停用。
  - 算完會在日誌顯示距離、服務估計時間與節點數。**實際模擬時間仍依你設定的移動速度**，
    不是服務估計的那個時間。
- **模擬移動中會鎖住編輯**，地圖上會顯示提示橫幅，避免走到一半路線被改掉。按「停止」即解鎖。
  此時「規劃路徑」與「簡化目前路線」也會一併停用。
...
├── _internal/              # 程式本體與 Qt/Chromium 執行環境（要跟 exe 放在同一層，不要單獨搬走）
...
- **打包機需要一份 OpenSSL 3（x64）的 DLL**（`libssl-3-x64.dll`、`libcrypto-3-x64.dll`），
  ...
  [Git for Windows](https://git-scm.com/download/win) 的話會自動找到它附的那份，不用另外設定；
  想用別的版本就把 DLL 所在資料夾設成環境變數 `PINDRIFT_OPENSSL_DIR`。都找不到時打包會直接中止。
